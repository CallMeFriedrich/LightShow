...
class ShowEngine:
    ...
    def render(
        ...
        # Build-up hebt die blitzigen Layer beschleunigend an (§9).
        eff_intensity = min(1.0, cfg.intensity * (0.35 + 0.65 * a.mood) * (1.0 + buildup))
        ctx = RenderContext(
            pixels=self.pixels, t=t, dt=dt, a=a, cfg=cfg,
            eff_intensity=eff_intensity, base_hue=self.base_hue,
            direction=self.scene.direction, drop=drop_eff, buildup=buildup,
            rng=self.rng,
        )
        canvas = self._canvas
        canvas[:] = 0.0
        ...
        if a.silence:
            ...
            self.scene.maybe_advance(t, a.mood, cfg.scene_seconds, self.pixels, cfg.section_floor)
        ...
        # Sektor-Maske (nach Drop: full-Override, §2).
        if drop_eff:
        ...
        self._out.brightness = cfg.brightness
